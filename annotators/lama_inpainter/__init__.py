...
def inpaint_preprocess(img: np.ndarray, mask: np.ndarray, inpaint_size=1024, device='cuda') -> np.ndarray:
    global model
    ...
    rel_pos, _, direct = model.load_masked_position_encoding(mask_torch[0][0].numpy())
...
model = None
@torch.no_grad()
def apply_inpaint(img, mask, device='cuda', precision=torch.float32):
    global model
    if model is None:
        model = load_lama_mpe(device=device, use_mpe=False, large_arch=True)

    im_h, im_w = img.shape[:2]
    img_torch, mask_torch, rel_pos, direct, img_original, mask_original, pad_bottom, pad_right = inpaint_preprocess(img, mask)
    

    if device in {'cuda'}:
        with torch.autocast(device_type=device, dtype=precision):
            img_inpainted_torch = model(img_torch, mask_torch, rel_pos, direct)
    else:
        img_inpainted_torch = model(img_torch, mask_torch, rel_pos, direct)
